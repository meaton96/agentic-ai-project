...
localize_anomaly.py
===================
Raw signal anomaly localization module.

Detects cross-cylinder imbalances in engine temperature sensors (EGT and CHT).
The methodology compares each cylinder's phase-averaged temperature against the 
median of its siblings. To prevent flagging benign, constant offsets, the 
algorithm isolates load-dependent faults by calculating the 'excess' deviation 
in a specific phase relative to the cylinder's calmest phase.
"""
...
def localize_anomaly(df: pd.DataFrame, segments=None, groups=None, thresholds=None,
    ...
    """
    Identifies specific cylinders that exhibit abnormal temperature deviations.
    
    Args:
        df (pd.DataFrame): Time-series data for a single flight.
        segments (list[dict], optional): Flight phase segments. If None, it will
            be computed using `segment_flight`. Defaults to None.
        groups (dict, optional): Mapping of sensor groups to channel lists. 
            Defaults to DEFAULT_GROUPS.
        thresholds (dict, optional): Excess deviation thresholds per group. 
            Defaults to DEFAULT_THRESHOLDS.
        sample_hz (float, optional): Sampling frequency. Defaults to 1.0.
        min_valid_frac (float, optional): Minimum required ratio of valid (non-NaN)
            data points for a channel to be evaluated. Defaults to 0.4.
            
    Returns:
        dict: A structured summary of detected anomalies, including the specific
            channels, deviations, worst phases, and whether the finding is 
            corroborated by another sensor group.
    """
