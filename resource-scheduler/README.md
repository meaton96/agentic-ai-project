...
5. **Optimization** — background policy-tuning loop, same shape as the ML pipeline's Phase-9 drift/retrain-decision subsystem. ✅ implemented (`steps/optimization_step.py`) — the odd one out: takes no task-table input at all, instead aggregating outcomes across recent runs by reading the report JSON files agents #2-4 already write to `runs/<run_id>/` (`environment/policy_evidence.py`). May only propose adjustments to parameters actually wired to a CLI flag (`queue_size`, `snapshot_window`, `slice_capacity`) — never an invented knob. Never applies its own proposal; sends it (with the real evidence dict, not just its own prose claim about it) to Human Oversight's mailbox — the third A2A hop. Short-circuits with zero LLM calls when there's no run history yet.
6. **Human Oversight** — approval gate on policy changes, same shape as the ML pipeline's Verification agent. ✅ implemented (`steps/oversight_step.py`) — reviews Optimization's proposal against the real underlying evidence (not just the proposer's own characterization of it). Same "degrade to flagged, never silently approve" rule as the ML pipeline's Verification agent, including when the LLM invents a verdict string outside the allowed enum. `scripts/run_human_oversight_agent.py --synthetic-proposal` allows isolated testing without agent #5.
